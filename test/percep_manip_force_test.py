...
class StretchController:
    def __init__(self):
        ...
        rospy.Subscriber('/force_states', Bool, self.force_callback)
        ...
        self.percep_start_flag = False
        self.force_flag = False
        self.initial_force_value = 53
        ...
        if self.percep_start_flag:
            try:
                ...
                if self.percep_count % 100 == 0:
                    ...
                    self.percep_start_flag = False  # Reset for the next cycle
    ...
    def run(self):
        rospy.loginfo("Initialising position...")
        publish_duration = rospy.Duration(10)  # 5 seconds
        start_time = rospy.Time.now()
        while rospy.Time.now() - start_time < publish_duration:
        ...
        self.percep_start_flag = True
        rospy.loginfo("Performing perception task...")
        while not self.percep_done_flag and not rospy.is_shutdown():
            rospy.sleep(0.1)

        rospy.loginfo("Performing manipulation task...")
        self.percep_start_flag = False
        print("Extending arm to align y-axis")
        self.percep_manip()
        print("Lowing arm until force is detected")
        self.manip_force()
